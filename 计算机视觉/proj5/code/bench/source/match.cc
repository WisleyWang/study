    ...
    Matrix& m1, Matrix& m2)
{
    ...
    for (int a = 0; a < (int)edges.size(); a++) {
        ...
        igraph(count,0) = i;
        igraph(count,1) = j;
		//rint does not exist in windows
        //igraph(count,2) = (int) rint (edges[a].w * multiplier);
		igraph(count,2) = (int) (edges[a].w * multiplier + 0.5);
        count++;
    }
    ...
    for (int a = 0; a < n; a++) {
		...
		const int w = (int)(sqrt((double)(dx*dx+dy*dy))*multiplier + 0.5);
        assert (w == c);
    }
        ...
        for (int y = 0; y < height; y++) {
            if (bmap1(y,x)) {
                if (match1(x,y) == Pixel(-1,-1)) {
                    ...
                    cost += 0.5 * sqrt ((double)(dx*dx + dy*dy));
                }
            }
            if (bmap2(y,x)) {
                if (match2(x,y) == Pixel(-1,-1)) {
