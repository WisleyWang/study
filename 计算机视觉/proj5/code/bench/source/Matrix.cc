...
#if NEXTPOW2
// excerpt from ieee754.h from Linux
...
#define DEFOP(OP,FUNC) \
	...
	if (a._rows != b._rows || a._cols != b._cols) { \
            throw Exception (String ( \
                "Matrix size mismatch in : (%d,%d) vs. (%d,%d).", \
                a._rows, a._cols, b._rows, b._cols)); \
	} \
        Matrix c (a._rows, a._cols, Matrix::undef); \
        for (int i = 0; i < a._n; i++) { \
...
Matrix::_alloc (int rows, int cols, FillType type) 
{
    ...
    switch (type) {
        ...
        case rand:
            _data = (double*) malloc (_n * sizeof(*_data));
            for (int i = 0; i < _n; i++) {
                _data[i] = Random::randdm.fp();
            }
            break;
        case randn:
            _data = (double*) malloc (_n * sizeof(*_data));
            for (int i = 0; i < _n; i++) {
                const double x = Random::randdm.fp();
                _data[i] = ::sqrt(-::log(x));
                if (Random::randdm.fp() > 0.5) {
                    _data[i] = -_data[i];
                }
            }
            break;
